...
\begin{itemize}
    ...
    \item $\epsilon_{t,s}$: Intraday price spread (stochastic noise) at time $t$ under scenario $s$ (\$/kWh).
    ...
    \item $\lambda^{sell}_{t,s}$: Price of selling electricity to the grid at time $t$ under scenario $s$ (\$/kWh). Defined as $\lambda^{sell}_{t,s} = \lambda^{DA}_t + \epsilon_{t,s}$.
\end{itemize}
