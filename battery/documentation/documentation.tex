...
\begin{equation}
    \min J = \sum_{t=1}^{N} \left[ \left( \lambda^{buy}_t P_{buy,t} - \lambda^{sell}_t P_{sell,t} \right) \Delta t + \alpha \left( P_{ch,t}^2 + P_{dis,t}^2 \right) + \epsilon \left( P_{buy,t}^2 + P_{sell,t}^2 \right) \right]
\end{equation}

\vspace{0.5cm}
\noindent \textbf{Note on Regularization:} The small quadratic term $\epsilon(P_{buy,t}^2 + P_{sell,t}^2)$ where $\epsilon = 10^{-6}$ acts as a numerical regularizer. It ensures that during periods of zero prices or high solar generation, the solver avoids non-zero "mirrored" buy and sell values (floating variables) by providing a slight incentive to keep grid interaction at zero.
...
\begin{equation}
\begin{aligned}
\min_{\mathbf{P}, \mathbf{E}} \quad & \sum_{t=1}^{N} \left[ \left( \lambda^{buy}_t P_{buy,t} - \lambda^{sell}_t P_{sell,t} \right) \Delta t + \alpha \left( P_{ch,t}^2 + P_{dis,t}^2 \right) + \epsilon \left( P_{buy,t}^2 + P_{sell,t}^2 \right) \right] \\
