...
const SkeletonCard: React.FC<{ height?: number; width?: string }> = ({
  height = 300,
  width = "100%",
}) => (
  <div
...
const Dashboard: React.FC<DashboardProps> = ({
    ...
    try {
      ...
      const timeDiffHours =
        result.telemetry.length > 1
          ? (new Date(result.telemetry[1].time).getTime() -
              new Date(result.telemetry[0].time).getTime()) /
            3600000
          : 1;
      ...
      const combined = result.telemetry.map((t: any) => {
        ...
        const buyCost = plan
          ? plan.expected_grid_buy_kw * t.price_buy_usd_per_kwh
          : 0;
        const sellRevenue = plan
          ? plan.expected_grid_sell_kw * t.price_sell_usd_per_kwh
          : 0;
        const profit = sellRevenue - buyCost;
        cumulativeProfit += profit;

        // Realized metrics based on Realized Spot prices (Intraday)
        const realizedBuyCost = plan
          ? plan.expected_grid_buy_kw * t.realized_price_buy_usd_per_kwh
          : 0;
        const realizedSellRevenue = plan
          ? plan.expected_grid_sell_kw * t.realized_price_sell_usd_per_kwh
          : 0;
        ...
        const alpha = 0.001;
        const degradation = plan
          ? alpha *
            (Math.pow(plan.cmd_charge_kw, 2) +
              Math.pow(plan.cmd_discharge_kw, 2))
          : 0;
        ...
        return {
          ...
          formattedTime: new Date(t.time).toLocaleString([], {
            month: "short",
            day: "numeric",
            hour: "2-digit",
          }),
      ...
      const dailyReturns = Object.entries(dailyAgg)
        .map(([date, val]) => ({
          date,
      ...
      const socDistribution =
        totalSoCPoints > 0
          ? [
              ...
              {
                name: "20-40%",
                percentage: (soc20to40 / totalSoCPoints) * 100,
              },
              {
                name: "40-60%",
                percentage: (soc40to60 / totalSoCPoints) * 100,
              },
              {
                name: "60-80%",
                percentage: (soc60to80 / totalSoCPoints) * 100,
              },
              {
                name: "80-100%",
                percentage: (soc80to100 / totalSoCPoints) * 100,
              },
            ]
          : [];
  ...
  const metrics = useMemo(() => {
    ...
    const totalBuyCost = data.combined.reduce(
      (acc: number, curr: any) => acc + (curr.buyCost || 0),
      0,
    );
    const totalSellRevenue = data.combined.reduce(
      (acc: number, curr: any) => acc + (curr.sellRevenue || 0),
      0,
    );

    const totalRealizedBuyCost = data.combined.reduce(
      (acc: number, curr: any) => acc + (curr.realizedBuyCost || 0),
      0,
    );
    const totalRealizedSellRevenue = data.combined.reduce(
      (acc: number, curr: any) => acc + (curr.realizedSellRevenue || 0),
      0,
    );

    const totalDegradation = data.combined.reduce(
      (acc: number, curr: any) => acc + (curr.degradation || 0),
      0,
    );
    ...
    const timeDiffHours =
      data.combined.length > 1
        ? (new Date(data.combined[1].time).getTime() -
            new Date(data.combined[0].time).getTime()) /
          3600000
        : 1;
    const totalChargeKwh =
      data.combined.reduce(
        (acc: number, curr: any) => acc + (curr.cmd_charge_kw || 0),
        0,
      ) * timeDiffHours;
    const totalDischargeKwh =
      data.combined.reduce(
        (acc: number, curr: any) => acc + (curr.cmd_discharge_kw || 0),
        0,
      ) * timeDiffHours;
  ...
  if (error)
    return (
      <div style={{ color: "#e74c3c", padding: "2rem", textAlign: "center" }}>
        {error}
      </div>
    );
  ...
  return (
    <div style={{ width: "100%", animation: "fadeIn 0.5s ease-out" }}>
      ...
      >
          ...
          >
            <span
              style={{
                ...
                fontWeight: "bold",
                color:
                  (metrics?.netRealizedProfit || 0) >= 0
                    ? "#38a169"
                    : "#e53e3e",
              }}
            >
              ${metrics?.netRealizedProfit.toFixed(2)}{" "}
              <span
                style={{
                  fontSize: "0.8rem",
                  fontWeight: "normal",
            ...
            <span style={badgeStyle(metrics?.profitVariance || 0)}>
              {metrics && metrics.profitVariance >= 0 ? "+" : ""}
              {metrics?.profitVariance.toFixed(2)} var
            </span>
          </div>
        ...
        <div style={metricCardStyle}>
          ...
          >
            ...
            >
              ${metrics?.totalRealizedBuyCost.toFixed(2)}{" "}
              <span
                style={{
                  fontSize: "0.8rem",
                  fontWeight: "normal",
            ...
            <span style={badgeStyle(metrics?.buyVariance || 0, true)}>
              {metrics && metrics.buyVariance >= 0 ? "+" : ""}
              {metrics?.buyVariance.toFixed(2)} var
            </span>
          </div>
          <div style={{ fontSize: "0.75rem", color: "#a0aec0" }}>
        ...
        <div style={metricCardStyle}>
          ...
          >
            ...
            >
              ${metrics?.totalRealizedSellRevenue.toFixed(2)}{" "}
              <span
                style={{
                  fontSize: "0.8rem",
                  fontWeight: "normal",
            ...
            <span style={badgeStyle(metrics?.sellVariance || 0)}>
              {metrics && metrics.sellVariance >= 0 ? "+" : ""}
              {metrics?.sellVariance.toFixed(2)} var
            </span>
          </div>
          <div style={{ fontSize: "0.75rem", color: "#a0aec0" }}>
      ...
      <section style={chartCardStyle}>
        <h2 style={{ marginTop: 0, fontSize: "1.1rem", color: "#2d3748" }}>
          Cumulative Net Profit Comparison (USD)
        </h2>
          ...
          <ResponsiveContainer>
            <AreaChart data={data.combined}>
              ...
              <XAxis
                ...
                stroke={AXIS_COLOR}
                fontSize={10}
                interval={Math.floor(data.combined.length / 6)}
              />
              ...
              <Area
                ...
                stroke="#319795"
                name="Realized Net Profit (Backtest)"
                strokeWidth={2}
                fillOpacity={1}
      ...
      <section style={chartCardStyle}>
        <h2 style={{ marginTop: 0, fontSize: "1.1rem", color: "#2d3748" }}>
          Market Spot Prices: Day-Ahead Plan vs. Realized Intraday (USD/kWh)
        </h2>
        <div style={{ width: "100%", height: 300 }}>
          <ResponsiveContainer>
            <LineChart data={data.combined}>
              ...
              <Line
                type="stepAfter"
                dataKey="price_buy_usd_per_kwh"
                stroke="#a0aec0"
                name="Day-Ahead Buy (Plan)"
                dot={false}
                strokeWidth={1.5}
                strokeDasharray="3 3"
              />
              <Line
                type="stepAfter"
                dataKey="realized_price_buy_usd_per_kwh"
                stroke="#e53e3e"
                name="Intraday Buy (Realized)"
                dot={false}
                strokeWidth={2}
              />
              <Line
                type="stepAfter"
                dataKey="realized_price_sell_usd_per_kwh"
                stroke="#38a169"
                name="Intraday Sell (Realized)"
                dot={false}
                strokeWidth={2}
              />
            </LineChart>
      ...
      {isGlobal ? (
        <>
          ...
          >
            ...
            <section style={chartCardStyle}>
              <h2
                style={{ marginTop: 0, fontSize: "1.1rem", color: "#2d3748" }}
              >
                Daily Returns Comparison (USD)
              </h2>
                    ...
                    <Bar dataKey="realizedProfit" name="Realized Profit">
                      {data.dailyReturns.map((entry: any, index: number) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            entry.realizedProfit >= 0 ? "#319795" : "#f56565"
                          }
                        />
                      ))}
                    </Bar>
            ...
            <section style={chartCardStyle}>
              <h2
                style={{ marginTop: 0, fontSize: "1.1rem", color: "#2d3748" }}
              >
                Battery SoC Level Distribution (Calendar Wear Risk)
              </h2>
              <div style={{ width: "100%", height: 250 }}>
                <ResponsiveContainer>
          ...
          >
            ...
            <section style={chartCardStyle}>
              <h2
                style={{ marginTop: 0, fontSize: "1.1rem", color: "#2d3748" }}
              >
                Battery State of Charge (SoC) Details
              </h2>
              <div style={{ width: "100%", height: 250 }}>
                <ResponsiveContainer>
                  <AreaChart data={data.combined}>
                    ...
                    <XAxis
                      ...
                      stroke={AXIS_COLOR}
                      fontSize={10}
                      interval={Math.floor(data.combined.length / 4)}
                    />
                    <YAxis stroke={AXIS_COLOR} fontSize={10} />
                    <Tooltip
                      formatter={(value: number) => [
          ...
          >
            ...
            <section style={chartCardStyle}>
              <h2
                style={{ marginTop: 0, fontSize: "1.1rem", color: "#2d3748" }}
              >
                Optimized Battery Dispatch Commands (kW)
              </h2>
              <div style={{ width: "100%", height: 250 }}>
                <ResponsiveContainer>
                  <BarChart data={data.combined}>
                    ...
                    <Bar
                      dataKey="cmd_charge_kw"
                      fill="#3182ce"
                      name="Charge Command"
                    />
                    <Bar
                      dataKey="neg_cmd_discharge_kw"
                      fill="#ed8936"
