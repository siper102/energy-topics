...
def load_data(setup_id: int = None) -> tuple[pd.DataFrame, BatteryParams]:
    ...
    with psycopg.connect(DB_DSN) as conn:
        ...
        if len(df_telemetry) < 2:
            raise ValueError(
                f"Not enough data points found ({len(df_telemetry)}) for setup_id {setup_id}. At least 2 points are required for optimization."
            )
...
def clear_dispatch_plans(setup_id: int = None):
    ...
    with psycopg.connect(DB_DSN) as conn:
        with conn.cursor() as cur:
            if setup_id:
                cur.execute(
                    "DELETE FROM dispatch_plans WHERE setup_id = %s;", (setup_id,)
                )
            else:
    ...
    try:
        with psycopg.connect(DB_DSN) as conn:
            with conn.cursor() as cur:
                ...
                update_cols = [
                    col
                    for col in df_db_ready.columns
                    if col not in ["target_time", "setup_id"]
                ]
                set_clause = ", ".join(
                ...
                conn.commit()
                print(
                    f"✅ Successfully saved {len(records)} optimal dispatch commands to DB (Atomic UPSERT)."
                )
